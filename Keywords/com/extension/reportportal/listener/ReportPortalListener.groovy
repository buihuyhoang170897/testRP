...
public class ReportPortalListener extends BaseListener implements ExecutionListenerEventHandler {
	...
	public beforeTestHandler(testInfo){
		this.executeAfterTestClosure()
		def res = testItem.startTestCase(this.getTestCaseName(testInfo), this.context.currentSuite,  this.context.currentLaunch,
	...
	private createLogSteps(stepId) {
		this.context.logSteps.each {
			def res = testItem.startNestedTestStep(it['name'], this.context.currentSuite,  this.context.currentLaunch)
			this.createDecriptionSteps(res.id,it['description'])
			//			this.logging.logInfo(stepId, it['name'])
			this.createErrorSteps(res.id)
			testItem.finishTestStep(res.id,Constant.REPORTPORTAL_PASS_STATUS)
		}
